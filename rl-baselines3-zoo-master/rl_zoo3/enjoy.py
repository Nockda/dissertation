...
def savecsv(action, obs, seed) -> None:
    ...
    action_df.to_csv(action_filename, index=False, header=False)

    # Export obs to CSV with the timestamped filename
    obs_df = pd.DataFrame(obs)
    obs_df.to_csv(obs_filename, index=False, header=False)
...
def enjoy(seed):  # noqa: C901
    ...
    # if len(action_vae)>0:
    #     print("action_vae의 크기는 ",len(action_vae))
    # else:
    #     print("action_vae가 비었습니다.")
    
    # if len(obs_vae)>0:
    #     print("obs_vae의 크기는 ",len(obs_vae))
    # else:
